...
<div class="mb-3">
    ...
    <div class="flex justify-between pl-1">
        <div class="flex gap-2">
            {#if isUpdating}
                <p class="text-white cursor-pointer text-xs hover:underline transition-all duration-300" on:click={cancelUpdate}>Cancel</p>
                {:else}
                <p class="text-white cursor-pointer text-xs hover:underline transition-all duration-300" on:click={setToUpdate}>Update</p>
            {/if}
            <p class="text-white cursor-pointer text-xs hover:underline transition-all duration-300" on:click={deleteComment}>Delete</p>
        </div>
